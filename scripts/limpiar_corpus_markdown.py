...
from scripts.agrupar_corpus_markdown import (
    encontrar_raiz_repo,
    parsear_front_matter_yaml,
)
...
def parsear_argumentos(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    cfg_def = raiz / "config" / "limpieza_corpus_valledellili.yaml"
    ent_def = raiz / "data" / "markdown" / "valledellili-org"
    sal_def = raiz / "data" / "processed" / "markdown_limpio" / "valledellili-org"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not config_path.is_absolute():
        config_path = (raiz / config_path).resolve()
    if not entrada.is_absolute():
        entrada = (raiz / entrada).resolve()
    if not salida.is_absolute():
        salida = (raiz / salida).resolve()
