...
4. **Lockfile**: Cursor ignora `*.lock` por defecto en indexacion; mantener `!uv.lock` donde aplique.
5. **Probar**: `git check-ignore -v <ruta>` para sintaxis tipo gitignore.
6. **Limites**: la terminal y MCP **no** quedan bloqueados por `.cursorignore`; `permissions.deny` afecta herramientas de archivo de Claude, no `cat` en Bash salvo sandbox.
7. **Backlog**: nunca volcar claves reales en tareas o documentacion de `backlog/`.
