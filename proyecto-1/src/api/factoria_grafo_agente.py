...
def _resolver_ruta_meta_prompt(cfg: Configuracion) -> Path:
    ...
    if raw.is_absolute():
        return raw
    return resolver_ruta_proyecto(raw)
...
def construir_grafo_agente_produccion(cfg: Configuracion) -> CompiledStateGraph:
    ...
    logger.info(
        "Grafo del agente compilado (router=%s, compositor=%s).",
        modelo_router,
        modelo_compositor,
    )
    return grafo
...
def construir_grafo_agente_produccion_o_none(
    cfg: Configuracion,
    ...
    if cfg.mock_llm:
        try:
            ...
            logger.exception(
                "Fallo inesperado al compilar el grafo del agente (MOCK_LLM): %s", exc
            )
            return None
    try:
