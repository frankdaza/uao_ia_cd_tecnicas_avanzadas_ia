...
ejecutable bajo ``proyecto-1/``. Los scripts y la API deben usar ``encontrar_raiz_workspace``
...
def encontrar_raiz_proyecto(inicio: Path | None = None) -> Path:
    """Sube directorios hasta hallar ``pyproject.toml`` (p. ej. ``proyecto-1/``)."""
...
def encontrar_raiz_workspace(inicio: Path | None = None) -> Path:
    """
    Raiz del workspace donde reside ``data/`` (hermano de ``proyecto-1/``).
...
def resolver_ruta_proyecto(rel: str | Path, inicio: Path | None = None) -> Path:
    """Ruta absoluta bajo la raiz del proyecto (``config/``, ``frontend/``, etc.)."""
    p = Path(rel)
    if p.is_absolute():
        return p.resolve()
