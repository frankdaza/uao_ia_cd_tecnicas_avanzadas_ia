...
Utilidades para descargar el sitio, generar el corpus Markdown e indexar en Qdrant. Consultas SQL ad hoc (p. ej. verificación de `chat_history` M2) están en `scripts/sql/`. Todos los comandos asumen el directorio **`proyecto-1/`** (donde están `pyproject.toml`, `src/` y `scripts/`). El corpus compartido vive en **`../data/`** en la raíz del workspace.
