"""
Ingesta de protocolos PDF: extraccion, chunking LangChain y upsert en Qdrant TAAM.
"""
...
_NOMBRE_ARCHIVO_PAYLOAD = "protocolo.pdf"
    ...
    with pdfplumber.open(ruta) as pdf:
        for indice, pagina in enumerate(pdf.pages, start=1):
            ...
            documentos.append(
                Document(
                    page_content=texto,
                    metadata={"pagina": indice, "nombre_archivo": _NOMBRE_ARCHIVO_PAYLOAD},
                )
            )
    ...
    *,
    ...
    if not fila.ruta_pdf or not fila.hash_pdf:
        ...
        return ResultadoIngesta(
            ...
            mensaje="Falta ruta_pdf o hash_pdf; no se puede indexar.",
        )
    ...
    if debe_omitir_ingesta(fila, forzar=forzar) and not forzar:
        return ResultadoIngesta(
            ...
            version=_version_efectiva(fila),
            mensaje=(
                "Ingesta omitida: indexacion_estado=ok y mismo PDF ya indexado "
    ...
    if not ruta.is_file():
        ...
        return ResultadoIngesta(
            ...
            chunks=0,
            version=_version_efectiva(fila),
            mensaje=f"PDF no encontrado en disco: {ruta}",
        )

    version = _version_efectiva(fila)
    try:
        documentos, paginas = extraer_documentos_desde_pdf(ruta)
        ...
        logger.info(
            "Ingesta TAAM ok tipo_id=%s paginas=%s chunks=%s version=%s",
            tipo_id,
            paginas,
            len(chunks),
            version,
